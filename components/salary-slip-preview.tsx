...
export function SalarySlipPreview({ employee }: any) {
  ...
  // Detect auto-generated slip (has attendance_summary from our generator)
  ...
  if (isAutoGenerated) {
    // Auto-generated: use stored values (daily rate = base/30)
    ...
    <div className="space-y-4">
      ...
      <div ref={slipRef} className="bg-white text-black rounded-lg border border-gray-300" style={{ backgroundColor: '#ffffff', color: '#000000' }}>
        <div className="pt-8 space-y-6 p-8">
          ...
          {/* Attendance Breakdown (auto-generated slips only) */}
          {isAutoGenerated && (
            <div className="overflow-x-auto">
              <table className="w-full text-xs md:text-sm border border-gray-300">
                <thead>
                  <tr className="bg-blue-600 text-white">
                    <th className="p-2 text-left border-b border-gray-300" colSpan={2}>Attendance Summary</th>
                  </tr>
                </thead>
                <tbody>
                  <tr className="border-b border-gray-300">
                    <td className="p-2">Working Days</td>
                    <td className="p-2 text-right font-semibold">{employee.working_days || 0}</td>
                  </tr>
                  <tr className="border-b border-gray-300">
                    <td className="p-2">Present Days</td>
                    <td className="p-2 text-right font-semibold text-green-700">{employee.present_days || 0}</td>
                  </tr>
                  <tr className="border-b border-gray-300">
                    <td className="p-2">Absent Days</td>
                    <td className="p-2 text-right font-semibold text-red-600">{employee.absent_days || 0}</td>
                  </tr>
                  <tr className="border-b border-gray-300">
                    <td className="p-2">Late Arrivals</td>
                    <td className="p-2 text-right font-semibold text-amber-600">{employee.lateDays || 0}</td>
                  </tr>
                  <tr className="border-b border-gray-300">
                    <td className="p-2">Early Outs</td>
                    <td className="p-2 text-right font-semibold text-orange-600">{employee.earlyOutDays || 0}</td>
                  </tr>
                  <tr className="border-b border-gray-300 bg-gray-50">
                    <td className="p-2 font-medium">Violation Deductions (3 late/early = 1 day)</td>
                    <td className="p-2 text-right font-semibold text-red-600">{employee.violationDeductions || 0} day(s)</td>
                  </tr>
                  {!isProbation && (
                    <tr className="border-b border-gray-300">
                      <td className="p-2">Leaves Used (from {14} annual)</td>
                      <td className="p-2 text-right font-semibold">{employee.leavesUsed || 0}</td>
                    </tr>
                  )}
                  {employee.absentSalaryDays > 0 && (
                    <tr className="border-b border-gray-300 bg-red-50">
                      <td className="p-2 font-medium">Absent Days Deducted from Salary</td>
                      <td className="p-2 text-right font-semibold text-red-600">{employee.absentSalaryDays} day(s)</td>
                    </tr>
                  )}
                  <tr className="border-b border-gray-300 bg-gray-50">
                    <td className="p-2 font-medium">Daily Rate (Base ÷ 30)</td>
                    <td className="p-2 text-right font-semibold">PKR {(employee.dailyRate || 0).toLocaleString("en-PK")}</td>
                  </tr>
                  <tr className="bg-red-50 font-semibold">
                    <td className="p-2">Total Salary Deduction ({employee.leaves_deducted || 0} days × PKR {(employee.dailyRate || 0).toLocaleString("en-PK")})</td>
                    <td className="p-2 text-right text-red-600">PKR {(employee.total_deductions || 0).toLocaleString("en-PK", { maximumFractionDigits: 0 })}</td>
                  </tr>
                </tbody>
              </table>
            </div>
